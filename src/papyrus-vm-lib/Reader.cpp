...
DebugInfo::DebugFunction Reader::FillDebugFunction() {
	...
	Fdebug.fnName = this->structure->stringTable.m_data[Read16_bit()];
...
ObjectTable::Object Reader::FillObject() {
	...
	Read32_bit();//	Dont remove!   size includes itself for some reason, hence size-4

	object.parentClassName = this->structure->stringTable.m_data[Read16_bit()];
	object.docstring = this->structure->stringTable.m_data[Read16_bit()];
	object.userFlags = Read32_bit();
	object.autoStateName = this->structure->stringTable.m_data[Read16_bit()];
...
VarValue Reader::FillVariableData() {
	...
	case Data.kType_Identifier:
		Data = VarValue( Data.kType_Identifier,this->structure->stringTable.m_data[Read16_bit()].data());
		break;
	case Data.kType_String:
		Data = VarValue(this->structure->stringTable.m_data[Read16_bit()].data());
		break;
...
ObjectTable::Object::PropInfo Reader::FillProperty() {
	...
	prop.docstring = this->structure->stringTable.m_data[Read16_bit()];
