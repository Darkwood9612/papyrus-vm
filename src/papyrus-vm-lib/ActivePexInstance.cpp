...
std::vector < ObjectTable::Object::VarInfo > ActivePexInstance::FillVariables(std::shared_ptr<PexScript> sourcePex, std::vector<std::pair<std::string, VarValue>> argsForFillPropertys) {
	...
	variableForState.name = "::State";// Creating temp variable for save State ActivePexInstance and transition between them 
	variableForState.typeName = "String";
	variableForState.value = VarValue(this->sourcePex->objectTable.m_data[0].autoStateName.c_str());
...
VarValue ActivePexInstance::GetElementsArrayAtString( const VarValue& array, uint8_t type) {
...
VarValue ActivePexInstance::StartFunction(FunctionInfo& function, std::vector<VarValue> &arguments) {
	...
	for (size_t line = 0; line < opCode.size(); ++line) {
		...
		if (opCode[line].first == FunctionCode::kOp_CallParent && opCode[line].second.size() > 3) {
		...
		case OpcodesImplementation::Opcodes::op_iAdd:
			...
			*opCode[line].second[0] = *opCode[line].second[1] + (*opCode[line].second[2]);
			break;
		...
		case OpcodesImplementation::Opcodes::op_iNeg:
			...
			*opCode[line].second[0] = *opCode[line].second[1] * -1;
			break;

		case OpcodesImplementation::Opcodes::op_fNeg:

			*opCode[line].second[0] = *opCode[line].second[1] * -1.0f;
			break;
		...
		case OpcodesImplementation::op_Cmp_eq:
			...
			*opCode[line].second[0] = VarValue((*opCode[line].second[1]) == (*opCode[line].second[2]));
			break;
		...
		case OpcodesImplementation::Opcodes::op_CallMethod: {
			...
			else *opCode[line].second[2] = parentVM->CallMethod(this, (IGameObject*)object, functionName.c_str(), argsForCall);
		}
		...
		case OpcodesImplementation::Opcodes::op_CallStatic: {
			...
			//const char* className = (const char*)(*opCode[line].args[0]);
			const char* functionName = (const char*)(*opCode[line].second[1]);

			*opCode[line].second[2] = parentVM->CallStatic("", functionName, argsForCall);
		}
			  break;
		...
		case OpcodesImplementation::Opcodes::op_StrCat:
			...
			OpcodesImplementation::strCat(*opCode[line].second[0], *opCode[line].second[1], *opCode[line].second[2], this->sourcePex->stringTable);
			break;
		...
		case OpcodesImplementation::Opcodes::op_Array_Create:
			...
			(*opCode[line].second[0]).pArray = std::shared_ptr<std::vector<VarValue>>(new std::vector<VarValue>);
			...
			case OpcodesImplementation::Opcodes::op_Array_Length:
				...
				if ((*opCode[line].second[1]).pArray != nullptr) {
					if ((*opCode[line].second[0]).GetType() == VarValue::kType_Integer)
						*opCode[line].second[0] = VarValue((int32_t)(*opCode[line].second[1]).pArray->size());

				}
				else *opCode[line].second[0] = VarValue((int32_t)0);
				break;

			case OpcodesImplementation::Opcodes::op_Array_GetElement:
				
				if ((*opCode[line].second[1]).pArray != nullptr) {
					*opCode[line].second[0] = (*opCode[line].second[1]).pArray->at((int32_t)(*opCode[line].second[2]));
				}
				else assert(0);
				break;
			...
			case OpcodesImplementation::Opcodes::op_Array_SetElement:
				...
				if ((*opCode[line].second[0]).pArray != nullptr) {
					(*opCode[line].second[0]).pArray->at((int32_t)(*opCode[line].second[1])) = *opCode[line].second[2];
				}
				else assert(0);
				break;
			...
			case OpcodesImplementation::Opcodes::op_Array_FindElement:
				...
				OpcodesImplementation::arrayFindElement(*opCode[line].second[0], *opCode[line].second[1], *opCode[line].second[2], *opCode[line].second[3]);
				break;

			case OpcodesImplementation::Opcodes::op_Array_RfindElement:
				
				OpcodesImplementation::arrayRFindElement(*opCode[line].second[0], *opCode[line].second[1], *opCode[line].second[2], *opCode[line].second[3]);
				break;
...
VarValue& ActivePexInstance::GetIndentifierValue(std::vector<std::pair<std::string, VarValue>>& locals, VarValue& value) {
	if (value.GetType() == VarValue::kType_Identifier && (const char*)value != nullptr) {
		std::string temp ;
